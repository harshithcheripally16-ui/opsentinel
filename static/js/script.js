document.addEventListener('DOMContentLoaded', () => {
    ...
    function hidePreloader() {
        ...
        if (preloader) {
            console.log('[Opsentinel Trace] UI Hydration complete: Preloader dismissed.');
    ...
    async function updateMetrics() {
        ...
        try {
            ...
            data = res.data; 
            console.log('[Opsentinel Trace] API Response:', data);
            setOnlineUI();
        } catch (error) {
            console.error('[Opsentinel Trace] Telemetry fetch failed:', error);
    ...
    async function fetchSystemInfo() {
        try {
            ...
            console.error('[Opsentinel Trace] Failed to fetch system info:', error);
        }
    }
    ...
    if (isDemo) {
        console.log('[Opsentinel Trace] Demo mode detected. Skipping auth checks.');
    ...
    function evaluateLogs(data) {
        ...
        if (highUsage.length > 0) {
            ...
            }).catch(err => console.error("[Opsentinel Trace] Error pushing alert:", err));
        } else {
